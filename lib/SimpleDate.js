...
module.exports = function (holidays, i18n) {
    ...
    SimpleDate.prototype.prettyPrint = function (formatStr) {
        var tmp = formatStr.replace("%B", this.monthName())
                            .replace("%b", this.monthNameAbbr())
                            .replace("%d", this.day)
                            .replace("%m", this.month)
                            .replace("%Y", this.year)
                            .replace("%A", this.weekDayName())
                            .replace("%a", this.weekDayNameAbbr());
        return tmp;
    };

    SimpleDate.prototype.monthName = function () {
        return _monthNames[this.month]
    }

    SimpleDate.prototype.monthNameAbbr = function () {
        return _monthNamesAbbr[this.month]
    }

    SimpleDate.prototype.weekDayName = function () {
        return _weekDays[this.weekDayIndex()]
    }

    SimpleDate.prototype.weekDayNameAbbr = function () {
        return _weekDaysAbbr[this.weekDayIndex()]
    }
    ...
    SimpleDate.prototype.isHoliday = function () {
        return _holidays[this._toMonthDayString()] || _holidays[this.toString()] || _holidays[this.weekDayIndex()]
    };
    ...
    SimpleDate.prototype.daysFromTodayAsText = function (longForm) {
